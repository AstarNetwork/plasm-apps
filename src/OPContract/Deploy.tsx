...
interface Props extends ContractModalProps, ApiProps, RouteComponentProps {
  codeHash?: string;
...
interface State extends ContractModalState {
  ...
  isHashValid: boolean;
  params: any[];
  operateParameters: StakingParameters;
}
...
class Deploy extends ContractModal<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.defaultState = {
      ...
      isHashValid: false,
      params: [],
      operateParameters: StakingParameters.default(),
  ...
  private static getContractAbiState = (
  ...
  private static getConstructorState = (contractAbi: Abi | null = null, ci = 0): Pick<State, never> => {
    ...
    const constructOptions: ConstructOptions = constructors.map((constr, index) => {
      return {
        key: `${index}`,
        text: <MessageSignature asConstructor message={constr} />,
        value: `${index}`,
      };
    });
    ...
    return (
      <>
        ...
        <Dropdown
          ...
          isDisabled={isBusy}
          isError={!isHashValid}
          label={"code for this contract"}
          onChange={this.onChangeCode}
          options={codeOptions}
          value={codeHash}
        />
        {this.renderInputName()}
        {isAbiSupplied ? null : this.renderInputAbi()}
        {contractAbi ? (
          <Dropdown
        ...
        <InputBalance
          ...
          value={endowment}
        />
        <InputParameters isDisabled={isBusy} onChange={this.onChangeOperateParameters} onEnter={this.sendTx} />
        {this.renderInputGas()}
      </>
    );
  };
  ...
  private onChangeCode = (codeHash: string): void => {
    this.setState(Deploy.getCodeState(codeHash));
  };
  ...
  private onChangeConstructorIndex = (constructorIndexString: string): void => {
    ...
    this.setState(Deploy.getConstructorState(contractAbi, constructorIndex));
  };
  ...
  private onChangeOperateParameters = (operateParameters?: any): void => {
    if (!operateParameters) {
      this.setState({ operateParameters: StakingParameters.default() });
      return;
    }
...
export default withMulti(withRouter(Deploy), withApi);
