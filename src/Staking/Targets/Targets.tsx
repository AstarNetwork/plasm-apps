...
function extractInfo(
  ...
  const validators = sortValidators(
    electedInfo.info.map(
      ({ accountId, exposure, validatorPrefs }): ValidatorInfo => {
        const stakers = exposure;
        const _exposure = stakers || {
        ...
        const bondOwn = _exposure.own.unwrap();
        const bondTotal = _exposure.total.unwrap();
        ...
        const rewardPayout = rewardSplit.gtn(0) ? amount.mul(rewardSplit).div(amount.add(bondTotal)) : new BN(0);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const isNominating = _exposure.others.reduce((isNominating: any, indv: any): boolean => {
        ...
        return {
          ...
          key,
          commissionPer:
            ((prefs as ValidatorPrefs).commission?.unwrap() || new BN(0)).muln(10000).div(PERBILL).toNumber() / 100,
          numNominators: _exposure.others.length,
          rankBondOther: 0,
          rankBondOwn: 0,
          rankBondTotal: 0,
          rankComm: 0,
...
function Targets({ className, sessionRewards }: Props): React.ReactElement<Props> {
  ...
  const [{ validators }, setWorkable] = useState<AllInfo>({ totalStaked: new BN(0), validators: [] });
  const [{ sorted, sortBy, sortFromMax }, setSorted] = useState<{
    sorted: ValidatorInfo[];
    sortBy: SortBy;
    sortFromMax: boolean;
  }>({ sorted: [], sortBy: "rankOverall", sortFromMax: true });
  ...
  const _sort = (newSortBy: SortBy, unsorted = validators, isAdjust = true): void => {
    ...
    setSorted({
      ...
      sorted: unsorted
        .sort((a, b): number => (newSortFromMax ? a[newSortBy] - b[newSortBy] : b[newSortBy] - a[newSortBy]))
        .sort((a, b): number => (a.isFavorite === b.isFavorite ? 0 : a.isFavorite ? -1 : 1)),
    });
  };
      ...
      {sorted.length ? (
        <>
          <InputBalance
            ...
            help={"The amount that will be used on a per-validator basis to calculate rewards for that validator."}
            isFull
            label={"amount to use for estimation"}
            onChange={setAmount}
            value={_amount}
          />
          <Table>
            <Table.Head>
              ...
              <th>&nbsp;</th>
              <th>&nbsp;</th>
              {["rankComm", "rankBondTotal", "rankBondOwn", "rankBondOther", "rankOverall"].map(
                (header): React.ReactNode => (
                  ...
                  >
                    <Icon name={sortBy === header ? (sortFromMax ? "chevron down" : "chevron up") : "minus"} />
                  </th>
                )
              )}
              <th>&nbsp;</th>
            ...
            <Table.Body>
              {sorted.map(
                (info): React.ReactNode => (
                  <Validator info={info} key={info.key} toggleFavorite={toggleFavorite} />
                )
              )}
            </Table.Body>
          </Table>
        ...
        <div className="tableContainer">{"Validator info not available"}</div>
      )}
    </div>
  );
}
