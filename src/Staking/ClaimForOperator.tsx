...
function ClaimForOperator({ className, isVisible }: Props): React.ReactElement<Props> {
  const { api } = useApi();
  ...
  useEffect((): void => {
    if (senderId && oldestEra <= era && era <= currentEra) {
      setExtrinsic(api.tx.dappsStaking.claimForOperator(era));
  ...
  return (
    <div className={`${className} ${!isVisible && "staking--hidden"}`}>
      <Button isPrimary key="claim-for-operator" label={"Claim"} icon="add" onClick={_toggleModal} />
      {isModalOpen && (
        <Modal dimmer="inverted" open>
          <Modal.Header>{"Claim for operator"}</Modal.Header>
          <Modal.Content>
            <InputAddress defaultValue={senderId} help={""} label={"Operator"} onChange={setSenderId} type="allPlus" />
            <Dropdown
...
export default ClaimForOperator;
