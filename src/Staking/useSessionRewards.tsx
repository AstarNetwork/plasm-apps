...
function toJSON(sessions: SessionRewards[], maxSessions: number): Serialized[] {
  return sessions
    .map(
      ({ blockHash, blockNumber, isEventsEmpty, parentHash, reward, sessionIndex, slashes, treasury }): Serialized => ({
...
function mergeResults(sessions: SessionRewards[], newSessions: SessionRewards[]): SessionRewards[] {
  const tmp = sessions.concat(newSessions).sort((a, b): number => a.blockNumber.cmp(b.blockNumber));
...
async function loadSome(api: ApiPromise, fromHash: BlockHash, toHash: H256): Promise<SessionRewards[]> {
  ...
  const headers: any = await Promise.all(
    results.map(({ block }: any): Promise<Header> => api.rpc.chain.getHeader(block))
  );
  const events: EventRecord[][] = await Promise.all(
    results.map(
      ({ block }: any): Promise<EventRecord[]> =>
  ...
  const rewards: [Balance | undefined, Balance | undefined][] = events.map((info): [
    Balance | undefined,
    Balance | undefined
  ] => {
  ...
  return results
    .filter(({ changes }: any): boolean => !!(changes && changes.length))
    .map(
      ({ changes: [[, value]] }: any, index: any): SessionRewards => ({
...
export default function useSessionRewards(maxSessions: number): SessionRewards[] {
  ...
  useEffect((): void => {
    ...
    const savedNumber = workQueue[workQueue.length - 1] ? workQueue[workQueue.length - 1].blockNumber : undefined;
        ...
        async (): Promise<void> => {
          ...
          while (true) {
            ...
            if (
              ...
              !lastNumber ||
              fromNumber.eqn(1) ||
              (workQueue.length >= maxSessionsStore && fromNumber.lt(savedNumber || lastNumber))
            ) {
              break;
            }
          }
