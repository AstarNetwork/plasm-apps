...
function extractStake(values: [Hash, Exposure][], divisor: BN): LineData {
  return [values.map(([, { total }]): number => balanceToNumber(total.unwrap(), divisor))];
}
...
function extractSplit(values: [Hash, Exposure][], validatorId: string): SplitData | null {
  ...
  return [{ accountId: validatorId, isOwn: true, value: last.own.unwrap() }]
    .concat(
      last.others.map(({ who, value }): { accountId: string; isOwn: boolean; value: Balance } => ({
    ...
    .map(
      ({ accountId, isOwn, value }): SplitEntry => {
        const label = toShortAddress(accountId);
        const percentage = value.muln(10000).div(total).toNumber() / 100;
        const tooltip = `${formatBalance(value, { forceUnit: "-", withSi: false })} ${currency} (${percentage.toFixed(
          2
        )}%)`;
...
export default function Validator({ className, sessionRewards, validatorId }: Props): React.ReactElement<Props> {
  ...
  const [{ rewardsChart, rewardsLabels }, setRewardsInfo] = useState<{
    rewardsChart: LineData | null;
    rewardsLabels: string[];
  }>({ rewardsChart: null, rewardsLabels: [] });
  const [{ splitChart, splitMax }, setSplitInfo] = useState<{ splitChart: SplitData | null; splitMax: number }>({
    splitChart: null,
    splitMax: 100,
  });
  const [{ stakeChart, stakeLabels }, setStakeInfo] = useState<{ stakeChart: LineData | null; stakeLabels: string[] }>({
    stakeChart: null,
    stakeLabels: [],
  });
    ...
    if (blockCounts) {
      ...
      sessionRewards.forEach(({ blockNumber, reward, sessionIndex, slashes }, index): void => {
        ...
        const totalBlocks = blockCounts
          .filter((count, countIndex): boolean => !!count && countIndex >= lastRewardIndex && countIndex < index)
          .reduce((total, count): BN => total.add(count), new BN(0));

        // calculate the rewards based on our total share
        const pos = reward.mul(totalBlocks).div(blockNumber.sub(sessionRewards[lastRewardIndex].blockNumber));
  ...
  useEffect((): void => {
    setBlocksLabels(sessionRewards.map(({ sessionIndex }): string => formatNumber(sessionIndex)));
  }, [sessionRewards]);
  ...
  return (
    <Columar className={className}>
      <Column emptyText={"Loading block data"}>
        {(rewardsChart || blocksChart) && (
          <>
            {blocksChart && (
              <div className="staking--Chart">
                <h1>{"blocks produced"}</h1>
                <Chart.Line
                  colors={COLORS_BLOCKS}
                  labels={blocksLabels}
                  legends={["blocks", "average"]}
                  values={blocksChart}
                />
              </div>
            )}
            {rewardsChart && (
              <div className="staking--Chart">
                <h1>{"rewards & slashes"}</h1>
                <Chart.Line
                  colors={COLORS_REWARD}
                  labels={rewardsLabels}
                  legends={[`${currency} slashed`, `${currency} rewards (est.)`, `${currency} average`]}
                  values={rewardsChart}
                />
              </div>
            )}
      ...
      <Column emptyText={"Loading staker data"}>
          ...
          <>
            {stakeChart && stakeChart[0].length !== 0 && (
              <div className="staking--Chart">
                ...
                <Chart.Line
                  labels={stakeLabels}
                  legends={[`${currency} total`, `${currency} own`, `${currency} other`]}
                  values={stakeChart}
                />
              </div>
            )}
            {splitChart && (
              <div className="staking--Chart">
                <h1>{"staker percentages"}</h1>
                <Chart.HorizBar aspectRatio={2} max={splitMax} values={splitChart} />
              </div>
            )}
          </>
        )}
