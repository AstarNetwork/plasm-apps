...
class BondExtra extends TxComponent<Props, State> {
  ...
  public componentDidUpdate(prevProps: Props, prevState: State): void {
    ...
    if (balances_fees !== prevProps.balances_fees || hasLengthChanged) {
      this.setMaxBalance();
    }
  }

  public render(): React.ReactNode {
    const { isOpen, onClose, stashId } = this.props;
    ...
    return (
      <Modal className="staking--BondExtra" header={"Bond more funds"} open size="small">
        {this.renderContent()}
  ...
  private renderContent(): React.ReactNode {
    const { stashId, systemChain } = this.props;
    ...
    return (
      <Modal.Content className="ui--signer-Signer-Content">
        ...
        <InputBalance
          ...
          label={"additional bonded funds"}
          maxValue={maxBalance}
          onChange={this.onChangeValue}
          onEnter={this.sendTx}
          withMax={!isUnsafeChain}
        />
        <ValidateAmount accountId={stashId} onError={this.setAmountError} value={maxAdditional} />
      </Modal.Content>
    );
  }
    ...
    this.setState(
      (prevState: State): State => {
        ...
        const {
          ...
          maxAdditional = prevState.maxAdditional,
          maxBalance = prevState.maxBalance,
        } = newState;
        const extrinsic =
          maxAdditional && maxAdditional.gte(ZERO) ? api.tx.dappsStaking.bondExtra(maxAdditional) : null;
  ...
  private setMaxBalance = (): void => {
    ...
    while (!prevMax.eq(maxBalance)) {
      prevMax = maxBalance;
      extrinsic = maxAdditional && maxAdditional.gte(ZERO) ? api.tx.dappsStaking.bondExtra(maxAdditional) : null;
...
export default withMulti(BondExtra, withApi, withCalls<Props>("derive.balances.fees"));
