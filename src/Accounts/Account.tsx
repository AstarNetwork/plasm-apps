...
function Account({ address, className, filter, isFavorite, toggleFavorite }: Props): React.ReactElement<Props> | null {
  ...
  const [{ isDevelopment, isEditable, isExternal }, setFlags] = useState({
    isDevelopment: false,
    isEditable: false,
    isExternal: false,
  });
  ...
  return (
    <tr className={className}>
      ...
      <td className="top">
        ...
        {isForgetOpen && (
          <Forget address={address} onForget={_onForget} key="modal-forget-account" onClose={toggleForget} />
        )}
        {isIdentityOpen && <Identity address={address} key="modal-identity" onClose={toggleIdentity} />}
      ...
      <td className="top">
        <AddressInfo address={address} withBalance={false} withExtended />
      </td>
      <td className="number top">
        <Button
          ...
          size="small"
          tooltip={"Send funds from this account"}
        />
        <Popup
          ...
          trigger={<Button icon="setting" onClick={toggleSettings} size="small" />}
        >
          <Menu vertical text onClick={toggleSettings}>
            <Menu.Item disabled={!api.api.tx.identity?.setIdentity} onClick={toggleIdentity}>
              {"Set on-chain identity"}
            </Menu.Item>
            <Menu.Item disabled={!isEditable || isExternal} onClick={toggleDerive}>
              {"Derive account via derivation path"}
            </Menu.Item>
            <Menu.Item disabled={!isEditable || isExternal || isDevelopment} onClick={toggleBackup}>
              {"Create a backup file for this account"}
            </Menu.Item>
            <Menu.Item disabled={!isEditable || isExternal || isDevelopment} onClick={togglePassword}>
              {"Change this account's password"}
            </Menu.Item>
            <Menu.Item disabled={!isEditable || isDevelopment} onClick={toggleForget}>
              {"Forget this account"}
            </Menu.Item>
            {api.api.tx.recovery?.createRecovery && (
              <>
                <Menu.Divider />
                {!recoveryInfo && <Menu.Item onClick={toggleRecoverSetup}>{"Make recoverable"}</Menu.Item>}
                <Menu.Item onClick={toggleRecoverAccount}>{"Initiate recovery for another"}</Menu.Item>
              </>
            )}
            {!api.isDevelopment && (
              <>
      ...
      <td className="mini top">
        <LinkPolkascan className="ui--AddressCard-exporer-link" data={address} type="address" withShort />
      </td>
    </tr>
  );
}
