...
function Overview({ className, onStatusChange }: Props): React.ReactElement<Props> {
  ...
  return (
    <div className={className}>
      <Banner />
      {isCreateOpen && <CreateModal onClose={_toggleCreate} onStatusChange={onStatusChange} />}
      {isImportOpen && <ImportModal onClose={_toggleImport} onStatusChange={onStatusChange} />}
      {isQrOpen && <QrModal onClose={_toggleQr} onStatusChange={onStatusChange} />}
      <Button.Group>
        <Button icon="add" isPrimary label={"Add account"} onClick={_toggleCreate} />
        <Button.Or />
        <Button icon="sync" isPrimary label={"Restore JSON"} onClick={_toggleImport} />
        <Button.Or />
        <Button icon="qrcode" isPrimary label={"Add via Qr"} onClick={_toggleQr} />
        {isLedger() && (
          <>
            <Button.Or />
            <Button icon="question" isPrimary label={"Query Ledger"} onClick={queryLedger} />
          </>
        )}
      </Button.Group>
      {hasAccounts ? (
        <>
          <div className="filter--tags">
            <Input autoFocus isFull label={"filter by name or tags"} onChange={setFilter} value={filter} />
          </div>
          <Table>
            <Table.Body>
              {sortedAccounts.map(
        ...
        "no accounts yet, create or import an existing"
      )}
    </div>
  );
}
