...
import { Icon, Tooltip, TxButton, CryptoType, Label } from "@polkadot/react-components";
...
function renderUnlocking({ address, stakingInfo }: Props): React.ReactNode {
  ...
  return (
    <div>
      ...
      <Icon name="info circle" data-tip data-for={`${address}-unlocking-trigger`} />
    </div>
  );
}
      ...
      {validatorPrefsDisplay.validatorPayment &&
        ...
        (((stakingInfo.validatorPrefs as any) as ValidatorPrefsTo145).validatorPayment ? (
          ...
          <>
            <Label label={"commission"} />
            <span>
              {(stakingInfo.validatorPrefs.commission.unwrap().muln(10000).div(PERBILL).toNumber() / 100).toFixed(2)}%
            </span>
          </>
        ))}
    </>
  ...
  const allItems = (
    <>
      {balancesAll && balanceDisplay.total && (
        <>
          <Label label={"total"} />
          <FormatBalance className="result" value={balancesAll.votingBalance} />
        </>
      )}
      {balancesAll && balanceDisplay.available && (
        <>
          <Label label={"transferrable"} />
          <FormatBalance className="result" value={balancesAll.availableBalance} />
        </>
      )}
      {balanceDisplay.vested && balancesAll?.isVesting && (
        <>
          <Label label={"vested"} />
          <FormatBalance className="result" value={balancesAll.vestedBalance} />
        </>
      )}
      {balanceDisplay.locked && balancesAll && (isAllLocked || balancesAll.lockedBalance.gtn(0)) && (
        <>
          <Label label={"locked"} />
          <FormatBalance className="result" value={isAllLocked ? "all" : balancesAll.lockedBalance}>
            <Icon name="info circle" data-tip data-for={`${address}-locks-trigger`} />
            <Tooltip
              text={balancesAll.lockedBreakdown.map(
                ({ amount, reasons }, index): React.ReactNode => (
                  <div key={index}>
                    {amount.isMax() ? "all available" : formatBalance(amount, { forceUnit: "-" })}
      ...
      {balanceDisplay.reserved && balancesAll?.reservedBalance?.gtn(0) && (
        <>
          ...
          <FormatBalance className="result" value={balancesAll.reservedBalance} />
        </>
      )}
      {balanceDisplay.bonded && (ownBonded.gtn(0) || otherBonded.length !== 0) && (
        <>
          <Label label={"bonded"} />
          <FormatBalance className="result" value={ownBonded}>
      ...
      {balanceDisplay.redeemable && stakingInfo?.redeemable?.gtn(0) && (
        <>
          ...
          <FormatBalance className="result" value={stakingInfo.redeemable}>
    ...
    return (
      <>
        ...
        <details>
          ...
          <div className="body column">{allItems}</div>
        </details>
      </>
    );
  }
