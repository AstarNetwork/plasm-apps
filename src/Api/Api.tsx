...
export default function Api({ children, url }: Props): React.ReactElement<Props> | null {
  ...
  useEffect((): void => {
    ...
    injectedPromise.then((): void => setIsWaitingInjected(false)).catch((error: Error) => console.error(error));
  ...
  return (
    <ApiContext.Provider value={{ ...state, api, isApiConnected, isWaitingInjected }}>{children}</ApiContext.Provider>
  );
}
